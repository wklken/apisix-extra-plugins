-- This is a plugin modified from apisix/plugins/log-rotate.lua.
-- while the original plugin is designed for nginx access.log/error.log,
-- this plugin add some changes to support the file-logger/buffered-file-logger.
-- changes:
-- 1. log warn instead of error when the log file is not exist
--    see https://github.com/apache/apisix/pull/9751
--
...
local plugin_name = "file-log-rotate"
...
local function rename_file(log, date_str)
    ...
    -- the access log file may not exist, so we just log warn instead of error
    if err then
...
function _M.init()
    timers.register_timer("plugin#file-log-rotate", rotate, true)
end


function _M.destroy()
    timers.unregister_timer("plugin#file-log-rotate", true)
end
